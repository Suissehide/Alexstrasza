...
class ReponseController extends AbstractController
{
    ...
    {
        return $this->render('reponse/index.html.twig', ['reponses' => $reponseRepository->findAll()]);
    }
    ...
    /**
     * @Route("/{id}/vote", name="add_vote", methods="POST")
     */
    public function addVote(Request $request, Reponse $reponse) : Response
    {
        if ($request->isXmlHttpRequest()) {
            $diff = $request->request->get('diff');
            if ($diff == '1') {
                $reponse->addVote($this->getUser());
            } else {
                $reponse->removeVote($this->getUser());
            }
            $em = $this->getDoctrine()->getManager();
            $em->flush();
            return new Response();
